// Wrap everything to ensure DOM and Supabase are ready
window.onload = function () {
  ...
  async function save() {
    ...
    try {
      ...
      const payload = { name, passport, status };
  ...
  function search() {
    ...
    document.getElementById("ticketStatus").innerText = "Status: " + c.status;
  ...
  function viewMap() {
    const address =
      "New Seven Star Travel and Tourism, Opposite of City Exchange, 615 Zone 53, Building 85 Umm al Dome North Muaither, Doha, Qatar";
    window.open(
  ...
  // Expose functions globally so buttons in HTML can call them
  window.login = login;
  window.save = save;
